...
if [ "$DEVICE" = "US996Santa" ] || [ "$DEVICE" = "US996" ] || [ "$DEVICE" = "H918" ]; then
  DEV_NAME=ELSA
else
  DEV_NAME=ELSA
  DEV_ABOOT=US996
...
# make sure adb is working, add mktweaks, disable forced encryption, rctd and triton
...
replace_section init.$NAME_LOW.power.rc "service triton" " " "service triton /system/bin/triton\n   class main\n   user root\n   group system\n   socket triton-client stream 660 system system\n   disabled\n   oneshot\n";
# remove old import in case coming from older version
remove_line init.rc "import /init.blu_active.rc"
$H990_SIM
## System modifications
# make sure init.mktweaks.rc can run
mount -o rw,remount -t auto /system;
append_file /system/etc/init.qcom.post_boot.sh sys.post_boot.parsed "post_boot-mod";
