...
static const char laf_shortname[] = "laf2";
		...
		unsigned intf, unsigned alt)
{
	...
	if (ret) {
		dev->ep_in->desc = NULL;
		//joe temp ERROR(cdev, "config_ep_by_speed failes for ep %s, result %d\n",
		//joe temp 		dev->ep_in->name, ret);
		return ret;
	}
	ret = usb_ep_enable(dev->ep_in);
	if (ret) {
		//joe temp ERROR(cdev, "failed to enable ep %s, result %d\n",
		//joe temp 		dev->ep_in->name, ret);
		return ret;
	}
