...
module_param_named(lge_debug, fg_lge_debug, int, 0600
);
#endif
...
static int fg_common_hw_init(struct fg_chip *chip)
{
	...
	if(chip->lge_rid_disable) {
		rc = spmi_ext_register_writel(chip->pdev->ctrl, 2, 0x11D0, &unlock_val, 1);
		if (rc) {
			pr_err("failed to USB_ID current source register unlock %d\n", rc);
			return rc;
		}

		rc = spmi_ext_register_writel(chip->pdev->ctrl, 2, 0x11F1, &disable_val, 1);
		if (rc) {
...
{	int fg_power_get_property_ext(struct power_supply *psy,
	...
	chip->bms_psy_d.get_property = fg_power_get_property_ext;
	chip->bms_psy_d.set_property = fg_power_set_property_ext;
}
#else
