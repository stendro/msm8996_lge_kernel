				...
				struct snd_soc_jack *jack, int status, int mask)
{
    ...
    if( current_status_detect_water && switch_device == LGE_HEADSET && !strcmp(mbhc->sdev.name, LGE_SWITCH_NAME_AUX)) {
        ...
        if( current_status_button_IRQs ) {
            pr_debug("%s: disable button pressed/released IRQs becuase of water detected on USB[current_status_button_IRQs=%d] \n", __func__, current_status_button_IRQs);
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
        ...
        if( current_status_button_IRQs == false ) {
            pr_debug("%s: enable button pressed/released IRQs disabled by water detected on USB[current_status_button_IRQs=%d]\n", __func__, current_status_button_IRQs);
    ...
    switch(event) {
        ...
        case WCD_MBHC_DETECTED_WATER:
            ...
            if( checkMBHC->current_plug == MBHC_PLUG_TYPE_HEADSET ||
                ...
                pr_debug("%s: sent a removal event to user space \n", __func__);
            } else {
                // 1-0. no plug
